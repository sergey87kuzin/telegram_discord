...
from discord_messages.tasks import handle_image_telegram_message
from discord_messages.telegram_helper import bot, handle_start_message, handle_command, handle_message
    ...
    def post(self, request):
        ...
        if message and message.get("photo"):
            if not callback_query:
                callback_query = {}
            handle_image_telegram_message.apply_async([message, callback_query])
            bot.send_message(chat_id=message.get("chat", {}).get("id"), text="вижу картинку)")
            return Response(HTTPStatus.OK)
        translator = GoogleTranslator(source='auto', target='en')
        answer_text = "Творим волшебство"

        return handle_message(message, translator, request, answer_text, image_url)
