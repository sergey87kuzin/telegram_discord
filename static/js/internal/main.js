...
(function($) {
  $(function() {
    if (!$.cookie('smartCookies') && !localStorage.getItem("modal") && $("body").hasClass("display_form")) {
      ...
      setTimeout (openModalWelcom, 3000);
    }
...
$(function() {
    function showCookieModal() {
        $(".n-cookie-modal").addClass("active");
    }
    ...
    $(".n-cookie-modal__close").click(function() {
        $(".n-cookie-modal").removeClass("active");
        $('#retailcrm-consultant-app').css("bottom", "30px");
    ...
    let observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
            for (var i = mutation.addedNodes.length; i--;) {
                if (mutation.addedNodes[i] === document.getElementById('retailcrm-consultant-app') &&
                    $(".n-cookie-modal").hasClass("active")) {
                    if ($(window).width() < 768) {
                        $('#retailcrm-consultant-app').css("bottom", "170px");
                    } else {
                        $('#retailcrm-consultant-app').css("bottom", "130px");
                    }
                }
            }
        });
...
//рандомная фоновая картинка для ЛК
// $(document).ready(function() {
...
//       return rand;
//     }
//     let randomMainBg = randomInteger(1,2);
// });
...
$(document).ready(function() {
    if ($(".profile__promocodes") && localStorage.getItem('promocode')) {
        document.querySelectorAll(".promocodes__name").forEach((el) => {
            if (el.innerText === localStorage.getItem('promocode')) {
                const btn = $(el).closest(".promocodes__item").find(".promocodes__btn");
                btn.addClass("us");
                setTimeout(() => {
                    btn.next().fadeIn(250);
                }, 150);
    ...
    $(".promocodes__btn").click(function() {
        const promo = $(this).closest(".promocodes__item").find(".promocodes__name").text();
...
$(document).on('click', '.avl-map-open', function() {
    ...
    const content = $('.avl-map[data-tab="' + tab + '"]');
